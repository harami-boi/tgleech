  ...
  try:
    ...
    if file["mimeType"].startswith("application/vnd.google-apps"):
      ...
      try:
        ...
        # Download the file using streaming to avoid memory accumulation
        request = Gdrive.service.files().get_media( # type: ignore
          fileId=file_id, supportsAllDrives=True
        )

        # Write directly to file using chunked downloads
        ...
        with open(file_name, "wb") as f:
          ...
          while done is False:
            ...
            # Write only the NEW data from this chunk
            chunk_data = file_contents.getvalue()
            f.write(chunk_data)
            # Reset the buffer for the next chunk
            file_contents.seek(0)
            file_contents.truncate()
            # The saved bytes until now
            file_d_size = int(status.progress() * file_size)
