...
async def start(client, message):
  ...
  text = "Colab Leecher is running. Send /help or /settings for options."
...
async def telegram_upload(client, message):
  ...
  text = "Send download link(s). One per line."
...
async def drive_upload(client, message):
  ...
  text = "Send Google Drive link(s). One per line."
...
async def yt_upload(client, message):
  ...
  text = "Send YouTube/yt-dlp link(s). One per line."
...
async def bot_status(client, message):
  if message.chat.id == OWNER:
    await message.delete()
    msg = await message.reply_text("<i>Fetching System Status...</i>")
    status_text = sysINFO()
    await msg.edit_text(f"**System Status**\n{status_text}")
...
async def handle_url(client, message):
  global BOT
  logging.info(f"handle_url triggered! task_going: {BOT.State.task_going}, started: {BOT.State.started}")
