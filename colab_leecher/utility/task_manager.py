...
async def taskScheduler():
  ...
  if is_dir:
    ...
    for link in BOT.SOURCE:
      if is_telegram(link):
        ...
        ida = "🧲"
        Messages.caution_msg = "\n\n<i><b> Torrents may be restricted by your ISP</b>, proceed with caution !</i>"
  ...
  if ospath.exists(Paths.WORK_PATH):
    ...
    makedirs(Paths.WORK_PATH)
    makedirs(Paths.down_path)
  # Generate private or public link prefix for Dump channel
  Messages.link_p = str(DUMP_ID)[4:] if str(DUMP_ID).startswith("-100") else str(DUMP_ID)

  import subprocess as _sp
  try:
    _sp.run(
      ["aria2c", "-d", Paths.WORK_PATH, "-o", "Hero.jpg", Aria2c.pic_dwn_url],
      timeout=15,
      stdout=_sp.DEVNULL,
      stderr=_sp.DEVNULL,
    )
  ...
  Messages.task_msg += f"__[{BOT.Mode.type.capitalize()} {BOT.Mode.mode.capitalize()} as {BOT.Setting.stream_upload}]__\n\n"
