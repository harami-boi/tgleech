...
try:
  from moviepy.video.io.VideoFileClip import VideoFileClip # moviepy v1
except ImportError:
...
def sysINFO():
  ...
  # Use project root drive for disk usage (works on both Windows and Linux)
  disk_path = os.path.splitdrive(Paths.WORK_PATH)[0] or "/"
  if disk_path and not disk_path.endswith(os.sep):
    disk_path += os.sep
  disk_usage = psutil.disk_usage(disk_path)
  cpu_usage_percent = psutil.cpu_percent()

  string = "\n\n─────「 System Usage 」─────\n"
  string += f"\n🖥 **CPU Usage:** __{cpu_usage_percent}%__"
...
async def send_settings(client, message, msg_id, command: bool):
  ...
  text = "**CURRENT BOT SETTINGS:**"
  text += f"\n\n UPLOAD: <i>{BOT.Setting.stream_upload}</i>"
  text += f"\n SPLIT: <i>{BOT.Setting.split_video}</i>"
  text += f"\n VIDEO FORMAT: <i>{BOT.Options.video_out}</i>"
  text += f"\n VIDEO QUALITY: <i>{BOT.Setting.convert_quality}</i>"
  text += f"\n CONVERT: <i>{BOT.Setting.convert_video}</i>"
  text += f"\n CAPTION FONT: <i>{BOT.Setting.caption}</i>"
...
async def status_bar(down_msg, speed, percentage, eta, done, left, engine):
  ...
  text = (
    f"\nDownloading: [{bar}] {percentage:.1f}%\n"
    f"Speed: {speed} | Engine: {engine}\n"
    f"ETA: {eta} | Elapsed: {getTime((datetime.now() - BotTimes.start_time).seconds)}\n"
    f"Size: {done} / {left}"
  )
  try:
    ...
    # Only log warnings for un-changed messages to avoid clutter
    pass
  except Exception as e:
